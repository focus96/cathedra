...
class TelegramStudentsFeedbackController extends AdminController
{
    /**
     ...
     * @var string
     */
    protected $title = 'Обратная связь бота для студентов';
    ...
    {
        $grid = new Grid(new TelegramStudentsFeedback());
        ...
        $grid->is_answered('Отвечен')->display(function ($isAnswered) {
            $link = '';
            if($this->answer) {
                $link = "<a href='/admin/telegram-bot/students/feedback/send/{$this->id}'> Отправить</a>";
            }
    ...
    {
        $show = new Show(TelegramStudentsFeedback::findOrFail($id));
    ...
    {
        $form = new Form(new TelegramStudentsFeedback);
    ...
    public function send(TelegramStudentsFeedback $feedback)
    {
        $config = [
