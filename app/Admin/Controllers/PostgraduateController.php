...
class PostgraduateController extends Controller
{
    ...
    {
        $grid = new Grid(new Postgraduate);
        $grid->id('Ид');
    ...
    {
        $show = new Show(Postgraduate::findOrFail($id));
    ...
    {
        $form = new Form(new Postgraduate);
        ...
        if ($result = mysqli_query($link, "SELECT * FROM postgraduates WHERE is_public = 1")) {
            ...
            if ($row_cnt = mysqli_num_rows($result) >= 5) {
                ...
                $form->saving(function ($form) {
                    ...
                    return redirect('/admin/postgraduate')->with(compact('error'));
                });
            }
            /* закрытие выборки */
