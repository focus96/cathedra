        ...
        <div class="row justify-content-between align-items-center">
            ...
            <div class="col-lg-4 col-md-6 search-course-right section-gap">
                ...
                <form class="form-wrap" id="feedback" action="{{ route('feedback') }}" method="post">
                    ...
                    @if($errors->has('contact'))<div class="alert alert-danger" role="alert">{{ $errors->first('contact') }}</div>@endif
                    <textarea name="message" class="form-control" placeholder="Питання" id="" rows="3"></textarea>
                    @if($errors->has('message'))<div class="alert alert-danger" role="alert">{{ $errors->first('message') }}</div>@endif
                    <button class="primary-btn text-uppercase">Надіслати</button><br><br>
                    @if(session('messageFeedback'))<div class="alert alert-success" role="alert">{{ session('messageFeedback') }}</div>@endif
                </form>
